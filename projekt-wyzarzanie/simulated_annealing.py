...
def generate_neighbor_swap(permutation: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    """Tworzy sąsiada przez zamianę dwóch różnych zadań miejscami."""
    neighbor = permutation.copy()
    i, j = rng.choice(len(neighbor), size=2, replace=False)
...
def generate_neighbor_insert(permutation: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    source, target = rng.choice(len(permutation), size=2, replace=False)
    values = [int(x) for x in permutation]
...
def make_initial_permutation(
    ...
    """Zwraca rozwiązanie początkowe: wynik Quick NEH albo permutację losową."""
    if mode == "neh":
...
def cooling_temperature(
    ...
    """Wyznacza temperaturę dla iteracji."""
...
def simulated_annealing(
    ...
    """
    Uruchamia symulowane wyżarzanie dla problemu F||Cmax.

    Lepszy sąsiad jest akceptowany zawsze. Gorszy sąsiad może zostać
    zaakceptowany z prawdopodobieństwem exp(-delta / T), dzięki czemu algorytm
    może wychodzić z minimów lokalnych.
    """
    if iterations <= 0:
